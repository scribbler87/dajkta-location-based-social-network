...
public class NewEventActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (bitmapTemp != null) {
			...
			// throw new RuntimeException("Bitmap not initialized");
		}

		// When a user clicks on the 'choose a image' button, the user is
		// directed into the media folder to choose a image
		chooseImageBtn.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				// Launch an image choosing activity
				Intent imageChoosingIntent = new Intent(
						Intent.ACTION_PICK,
		...
		sendBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// TODO check start and endtime if they are the default
				// values????
				else if ("".equals(sUri)) {

				} else {
					throw new RuntimeException("Uncaught if clause");
				}
				sendNewEvent();
			}
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		case SELECT_PICTURE:
			if (resultCode == RESULT_OK) {
				...
				bitmap = yourSelectedImage;// Store the bitmap of image in case
											// of screen rotation
			} else {
				Log.d("Picture activity resultcode other than RESULT_OK",
						String.valueOf(resultCode));
			}
			break;
		default:
			throw new RuntimeException("Uncaught switch clause :" + requestCode);
		}

	}

	// Save the bitmap representation of the image when screen rotates and
	// restore it upon completion of rotation.
	@Override
